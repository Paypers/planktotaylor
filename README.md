...
- **Counting planks** ([src/lib/planks.ts](src/lib/planks.ts)): every plank held to the end counts once. A plank that counted for today's song and a ladder level is one plank (under today's song), and goes again (extra credit on today's song, practice on a climbed level) count too; they come from the attempt history, since the records only keep what counted for something. The Streak section's Planks and its time ring, and Your Plank Year, all count this way. The ring in Streak splits the time by what it was for, or by album (the top four, then the rest together, so no slice gets too thin to read). Its three colours (`--chart-daily`, `--chart-ladder`, `--chart-again` in styles.css) were checked as a colourblind-safe set in both themes; the list under it gives every value, so nothing relies on colour or hovering.
